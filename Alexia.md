...
> **Where a plugin's tab went (D118, 2026-09-02).** It is the second half of that plugin's own
> page now, under the settings that drive it, because a plugin with a tab here *and* a pane on
> the settings screen was two places to look for one thing and neither screen said which held
> what. `panel` did not move in the manifest and this milestone's gate did not move either —
> the plugins page was already assembled the same way, so *delete the folder and it goes* is
> still the test, one screen over.
...
| Date | Decision | Notes |
|---|---|---|
| 2026-09-15 | **D152 — a Mac update comes back on its own** | The first Mac update, 0.4.1 to 0.5.1, reached 100% and stayed there. It had worked: 0.5.1 was in `/Applications` 28 s after launch, and the process still running was the old app, which Tauri had moved to a temporary folder. `installUpdate` was written for Windows, where the NSIS installer ends the process and restarts it, so it had no success path; on a Mac the updater swaps the bundle and returns, and nothing ended anything. The page now calls a `relaunch` command once the install returns. The command is three lines of Rust around `request_restart`, which goes through `RunEvent::Exit` (core stopped, single-instance socket removed) and then starts the path Tauri recorded before `main`, where the new bundle now is. A test holds the page's call and the shell's command together. **It helps updates made from a build that has it**, so leaving 0.5.1 still needs one quit and reopen. Not yet run end to end: that needs a release newer than the build doing the updating. |
| 2026-09-15 | **D151 — an update offer points at its own release, not at `latest`** | The first Mac update failed with *Download request failed with status: 404 Not Found*. The app had checked at 09:52, while 0.5.0 was latest, and the button was pressed after 0.5.1 went out; `latest.json` named its downloads as `releases/latest/download/<name>`, so 0.5.0's `Alexia_darwin-aarch64.app.tar.gz` was looked for on 0.5.1, which has no such file. It was never Mac-specific. The Windows installer's name carries its version, and a name that stayed the same would have been worse: the newer bytes under the older signature, refused as tampering. A tray app holds an offer for as long as it runs, so this is the ordinary case, not a race. The manifest is still fetched from `latest`, which is how an install finds a new version; the URLs inside it now name the tag, so a stale offer installs exactly what it offered and the next start offers the rest. 0.5.1's own `latest.json` was re-uploaded with pinned URLs and unchanged signatures, so offers made from it survive 0.5.2. |
| 2026-09-15 | **D150 — one Mac download, a real icon, and a signing switch per platform** | **Universal.** 0.5.0 shipped a disk image per processor, which asked somebody who has never opened *About This Mac* which chip they have before they had seen anything work — setup dying on the download page. One Apple Silicon runner now builds both halves: Tauri merges the shell, and `sidecar.mjs --universal` merges Node and the keychain library with `lipo`, fetching the other processor's copy and refusing it unless it matches nodejs.org's SHASUMS256 or the lockfile's integrity. The library lands as `keyring.darwin-universal.node`, the first name its loader tries. Tauri also wants a sidecar under each processor's own triple, because it builds the halves separately before merging. The release fails if any binary comes out single-processor, and `latest.json` points both `darwin-aarch64` and `darwin-x86_64` at the one file, since each half's updater asks for its own. About twice the download. **The icon.** The only app icon was the 64 px tray sketch; Tauri packed it into the icns 48 px slot and macOS drew rainbow noise in the Dock. Both platforms' icons are now `alexia-mark.svg` in champagne on a cobalt tile, with the SVG sources beside them; the tray is unchanged. Tauri sets the Dock icon at runtime only in dev builds, so what a release shows is the bundle's. **`sign` became `sign_windows` and `sign_macos`**, because SignPath and a paid Apple account arrive separately and one switch held the first hostage to the second. Verified: a universal build made on Apple Silicon (96 MB disk image), both halves in every binary, a valid signature, and the core inside it starting, serving the shell and loading the merged keychain library. The Intel half is run by the release itself: a job on an Intel runner starts the core from inside the app about to be published, and publishing waits for it. Local builds on this machine need `SDKROOT` pointed at the 26.5 SDK: the 27.0 SDK's library stubs name an architecture the installed linker does not know. |
| 2026-09-14 | **D149 — two checks that went red at random were two real faults** | Invariant 11 failed five of eight runs around 0.5.0 and the Windows OCR suite failed two, one of them the 0.4.1 release, and both had been read as somebody else's weather. **The no-key floor stopped at a 406.** AI Horde says 406 when no volunteer awake can serve a model — *"Model None not known!"* for a seeded row whose worker had left, *"Request is not possible."* — and `send` read it as fatal, so a first message died with two Horde rows still behind it on exactly the minute OVHcloud's anonymous two requests were spent. A 406 is now a rung failure, beside 403. **A plugin gave up on another plugin at sixty seconds** because `alexia.capability()` took MCP's default timeout while core allows the provider two minutes: Windows OCR reading a scan on a cold runner takes 39–49 s and sometimes 60, and Telegram handing a long voice note to `voice` is the same shape. `CAPABILITY_CALL_MS` in `@alexia/protocol` is now the SDK's wait, held above core's start and call budgets by a test. Verified with a provider that takes 65 s: 60.3 s and the consumer's fallback sentence before, the answer at 65.6 s after. Reaching installed plugins needs `documents` and `telegram` published again, since each bundles the SDK. |
| 2026-09-14 | **D148 — computer control on a Mac** | `macos.js` answers every call `windows.js` does, through `osascript`'s JavaScript: Quartz events for pointer and keys, the accessibility tree for controls, `screencapture` for the picture — no native module. `desktop.js` picks the backend and imports nothing else, so the proof that a replayed plan cannot reach a model still walks the imports. Key notation is each backend's to state, so a Mac is taught `cmd+c` and refuses `^c` by name. Running it found that a string never crossed the bridge into a keystroke (*žluťoučký* arrived as `aaaaaaaaa`) — the characters now go as UTF-16 bytes — and that a letter must be looked up on the keyboard in use, so `cmd+z` undoes on a Czech layout. Verified on Calculator and TextEdit; the screenshot is not, for want of a Screen Recording permission on the test terminal. |
| 2026-09-14 | **D147 — the Mac voice programs are built here** | whisper.cpp publishes no Mac program and Piper's Apple Silicon archive is an Intel binary without its libraries. `scripts/voice-macos.mjs` builds both from the pinned tags, refuses to pack anything that loads a library from the build machine, and makes Piper speak before packing; `voice-macos.yml` drafts the release the plugin downloads from, never marked latest. Verified end to end on Apple Silicon through the plugin's own install: Piper speaks, Whisper hears it back. The Intel build, the workflow and the microphone are not. |
| 2026-09-14 | **D146 — text in pictures on a Mac, through Vision** | The recogniser the OS ships, reached from a script, as on Windows. Decoded from bytes rather than a path, because a page handed over by another plugin has no extension and AppKit picks decoders by extension. The whole scanned-PDF chain passes on a Mac. |
| 2026-09-14 | **D145 — the Mac app** | Option+Space, because `Ctrl+Option+Space` is macOS's input-source switch. Reopen brings the window back; `--overlay` summons the overlay from a command; the overlay joins every Space and sits over full-screen apps. The Dock icon follows the main window. The terminations blamed on toggling it were a quit-on-close utility on the test Mac. D66's blur guard, owed since M5-2, is built. The overlay does **not** appear over another app's full-screen Space — verified, not assumed — and the known fix is a non-activating NSPanel, which is unsafe Rust in the shell and waits on the owner. Hardened-runtime entitlements, a microphone sentence, the keychain library signed, a notarising Mac release job, `latest.json` for every platform in one publish job, and CI on macOS. Notarisation needs an Apple Developer account and is not verified. |
| 2026-09-14 | **D144 — Alexia runs on a Mac, and a fresh install was never watching its plugins folder** | Run on Apple Silicon, macOS 26.6. Core needed nothing — the suite and the invariants pass as they stand — so *portable by discipline* held. The packaging did not, silently, as D75 predicted of any build nobody has run: a keyring slug (`darwin-universal`) that has never been published, `node.exe` and `.cmd` on every platform, a browser opened with `cmd` whose failure is an event the `try` never heard, and a self-check that would have used the real data folder. Fixed, plus a `tauri.macos.conf.json` that builds an `.app`; no Rust changed. Running it also found a bug on every platform: `extensions` does not exist until something is installed, watching it failed once and never retried, and a plugin folder deleted by hand went unnoticed until restart. `serve()` now makes the folder before watching. Verified: `Alexia.app` launched, core behind it, page loaded, token guard holding. What it left is D145–D148. Corrected the same day: it first claimed a background launch shows no window — the window was on another desktop, and the window list only reports the one in front. |
| 2026-09-02 | **D143 — a plugin had two homes, and neither screen said which held what** | Reported from use. `plugins/voice` put the values you set on one screen and the things those values drive on another: the recording to clone and the words in it were typed under Settings → Plugins → Voice, and the button that reads them was a tab on the control surface — so two of that plugin's own hints had to end *on the plugins screen* and *on the Voice panel*, which is a manifest apologising for the shell. **The split was real and it was in the wrong place.** *Values you set* and *a record you read* are genuinely two things, and D86 made them two lists for that reason; what does not follow is two screens, because somebody opening either one is asking about **the plugin**, not about which half of it. So `panel` stays exactly where it is in the manifest — no revision, nothing to migrate, the three other manifests that declare one untouched — and its widgets are drawn under the settings on the plugin's own page, with a rule between them and the author's `label` above only where that is not the plugin's own name, because *Voice* under *Voice* is a section break that marks nothing. **The founding property is what made this cheap rather than risky**: the plugins page was already assembled from manifests exactly the way the tab list was, so no file in core or the shell gained a plugin's name, and deleting a folder still takes the whole plugin with it — the same M6-G test, one screen over. Three things fell out of it. The palette had **two rows for one plugin** and now has one, because `library` already listed every installed plugin and pointed at the page that now holds all of it. `Tab` lost `from`, `plugin` and `running`, `tabs()` stopped taking manifests, and `/api/panels` became a read of the store — the control surface is core's own and says so in its types. And **voice's manifest moved three widgets rather than being left correct-but-strange**: the clip, its transcript and the path of a downloaded voice now sit next to the buttons that read them, so a hint can say *above* and be right, which is the whole of what the change was for. The one thing deliberately not done is collapsing the two lists into one — that is a protocol revision and a migration for every plugin, to buy an ordering that an author can already have by choosing which list to declare in. |
| 2026-09-02 | **D142 — the two widgets that fill themselves were both unreachable** | `/api/rows` finds a widget by key and reads its tool, and the finder was a list of two type names. `image` (D132) and `cards` (D141) were never added to it, so core said *there is no list called that* for both — the picture gallery had been saying it since the day it shipped, on a panel nobody opened twice. The shell had a second fault on the same path: both sent the rows tool's name where the route wanted the widget's key. Fixed at the root — the finder is *anything with a `rows` tool*, and every widget asks by key — with an invariant over every manifest in the repo, proved by restoring the old rule and watching it name both. |
| 2026-09-02 | **D141 — `cards`, the fourteenth widget, and `alexia_protocol` 6** | D140 answered *browse the library* with a table and argued the bento grid was a look rather than a capability. Using it settled that the other way: forty spreadsheet rows are forty things nobody can judge, while a name, a sentence and a state is what people already read on the plugins page. Almost none of it is new drawing — `bento`, `bento-card`, `bento-label` and `dim` all existed; what is new is that the cards come from a plugin's tool rather than the folder, so core learns a plugin contains things and never learns the word *workflow*. `when` on a row action is the one real addition: Install belongs on what is not here and Remove on what is. At eleven rems the foot is laid out rather than left to wrap, or every card is three rows tall and the grid stops being scannable. |
| 2026-09-02 | **D140 — the library is a page, and it is the third tier only** | §8.4 ranks Verified, Curated and Templates. Two of them do not exist — nothing records what has been run and tagged, and no Alexia set is published — and §8.4 itself says a badge that records nothing rots into authority it stopped deserving. So the page is ComfyUI's own catalogue and says so. It needed no core work: the bento grid C2 proposed is a look, and `table` already carries rows from a tool, an action per row, a detail view, a filter and grouping, which is all of *browse and install*. Filtered on the machine first: 45 of 468, nothing paid, grouped by category, installed first. |
| 2026-09-02 | **D139 — a workflow may come from a link** | §8.6 forbade a URL from a conversation, and that rule was written because installing a node pack runs a stranger's Python. It is not the right rule for a workflow: the converter refuses any node this machine does not already have, so a link yields text that either maps onto what is installed or is refused by name — exactly as dangerous as the file on disk `add_workflow` already took. What is enforced instead is shape: https only, a size ceiling, and the commonest mistake named rather than reported, because a link to the page showing a workflow is not a broken workflow. Both sources were checked: comfy.org is the web front of the package ComfyUI ships, and its 626 against our 468 is a stale pin (0.11.9 from July; PyPI is at 0.11.52). RunComfy stays out — sign-in throughout, no public API, terms unstated. |
| 2026-09-02 | **D138 — the wiring answers what the titles do not** | D128 offers a field because its author named the box, and that stays. But ComfyUI’s own templates rename nothing, so every catalogue install arrived with no fields and ran with its author’s baked-in prompt. When nothing is titled the graph’s wiring is read instead: a sampler is any node taking both a positive and a negative link, which is which is stated by the node consuming them, and the text box is found by its declared type. Every ambiguity returns nothing, and the positive box is required — offering *avoid* alone claims steering that does not exist. Then it was run: SDXL Turbo installs, takes the prompt, and against a substituted checkpoint returns grey mush, because a Turbo workflow is one step at guidance 1 tuned to its own model. Without the swap the catalogue is unusable on this card; with it the picture can be ruined. So the swap is offered and its cost is stated where the choice is made. |
| 2026-09-02 | **D137 — installing a catalogue workflow needs no consent ladder** | §8.6 called this the sharpest edge in the plan: a workflow is harmless JSON, but the node packs it needs are a stranger's Python. Building it showed the risk is not reachable — the converter refuses any node class this machine does not already have, so anything that installs cleanly is something whose nodes are already here. Nothing is downloaded, nothing runs, one JSON file is written. Asking for consent to a power the tool does not have is worse than not asking: it teaches somebody the question is noise. What survives is the half that was never about consent — the page lists what was installed and can remove it. Measured: 468 entries, 45 runnable, SDXL Turbo installs by its title. It arrives with no settings, which is D128 working — ComfyUI's templates never rename their nodes. |
| 2026-09-02 | **D136 — the pipeline is a strip rather than a graph** | §7.3 of the engine plan gave up two things: a node could only be marked on or off, so per-node percentages were undrawable, and the force layout is built for a cloud of related notes while a workflow is a line. Both followed from assuming the diagram would be the `graph` widget, and **neither survives dropping it** — segments in execution order need no layout, and a segment's own width is the percentage the boolean could not carry. So the stages ride the progress notification beside D135's picture: no widget change, no protocol bump, the memory plugin untouched. The order is the plugin's, because ComfyUI names a node only once it has started — measured, not assumed — so core would be drawing the reporting rather than the pipeline. No names on it: five fit across the rail and twenty-five do not. A malformed strip is refused whole, since a `state` becomes a class name. Two faults surfaced only by running it: a finished job never closed its own strip, and D135's picture, shipped hours earlier, was crushing the step name it sat beside — one screenshot said what a working data path had hidden. |
| 2026-09-01 | **D135 — the picture, while it is still being made** | D129's websocket carries binary frames as well as JSON: the image as it stands, in two layouts read off ComfyUI's source. **The first attempt got none of them and the code was right** — `--preview-method` defaults to `none`, so there was nothing to send; Manager no longer overrides it at runtime, so the flag at launch is the only lever. `latent2rgb` rather than `auto`, because `auto` prefers a model that must be downloaded and then decoded on the card already busy generating. The frame is a `data:` URL and never a path — it exists for a second and is replaced — capped where it is produced, because a progress channel is not a transport. It rides under `_meta`, so **no protocol bump**: an older Alexia draws the bar and ignores the rest. Core accepts only a `data:image/` URL, since this is the one field a plugin fills that becomes an `img` source. The shell keeps one element and replaces it rather than building a filmstrip of a thing with one current state. Measured: 20 steps, 16 frames, 35 KB to 98 KB. |
| 2026-09-01 | **D134 — a converter that refuses instead of guessing, and the number that says how far it gets** | D123 refused to re-implement ComfyUI's conversion because one that guesses fails silently on exactly the interesting graphs. That still holds — but silence is not the only option: widget order is the catalogue's own `input_order`, so the expected count is a number and disagreeing with it is arithmetic. Every uncertainty refuses and names the node. **Measured against all 45 templates this card can run, it converts 6**; 25 stop on a subgraph, which the original analysis predicted as *one save away*. That figure is the finding: a hand-written conversion recovers about one in seven, and the rest still need the frontend. Six with proof beats none. It also uncovered a shipped bug — combos are spelled two ways on one install, and reading only one meant a field silently stopped validating. |
| 2026-09-01 | **D133 — ComfyUI ships the catalogue, and the honest number is not the big one** | Its own `/templates/index.json` lists 468 workflows with tags, required models and the video memory each wants — local, current, nothing to scrape. But two numbers in the plan were too flattering: **255 of them call paid services**, and of the 213 open-source ones only **45 fit an 8.5 GB card**. So the shelf is filtered on the machine before it is ranked on the words, and the count it leads with is the one about your card. *468 workflows* to somebody who can run 45 is accurate and misleading at once. The search also needed a relevance floor, found by using it rather than by reasoning: without one it answered *read this out loud* with depth estimation. |
| 2026-09-01 | **D132 — `image`, the thirteenth widget** | The twelve could describe a picture and not show one. Not the `file` widget refused at D89 — that was asking somebody to *choose* a file, which a browser cannot express; this is a plugin that has made something and has nowhere to put it. Granted on what the alternatives cost, the same argument `graph` won on, and arriving with three users rather than one. A plugin says what the pictures are; core owns how they look. Its route reads inside the asking plugin's own folder and nowhere else, resolving real paths on both sides first, because a widget must not become a file reader any plugin can aim anywhere. `alexia_protocol` 5; the floor stays at 2. |
| 2026-09-01 | **D131 — a conversation ending is something a plugin can be told** | Holding a graphics card between two pictures is helpful; holding it all night because nobody said stop is not. A new conversation is the clearest signal a person gives that they have finished, and until now there was no way to give it. Broadcast to every running plugin, carrying **nothing about the conversation** — a plugin that could tell two apart could keep a record of them. Nothing is spawned to hear it and nothing waits; ignoring it is right for almost every plugin. |
| 2026-09-01 | **D130 — the picture plugin is called *Local media generation*** | Deferred once already, until a workflow that is not a picture actually ran. The engine plan makes that certain rather than likely, so the name moves now and the engine is named in the summary instead of the title — the card reads for the person the setup promise is written for, and still says *ComfyUI* to anyone looking for it. The id and storage namespace stay `media`: neither migrates, and moving them would orphan every picture already made. |
| 2026-09-01 | **D129 — the websocket, adopted, overturning a comment that argued the other way** | The picture plugin chose polling on purpose — *a dependency and a reconnect story less* — and named its own successor in the same breath. Per-step progress exists only over the socket, and *step 12 of 20* is what somebody watching a render actually wants. `/history` still decides the job is done; the socket only makes waiting legible, is opened per job, and degrades to the old behaviour when it fails, so there is still no reconnect story. **The bug it was built around is silent**: ComfyUI sends progress only to the client that queued, so two different ids mean a socket that connects and never hears anything — indistinguishable from a slow card. One id per process, used by both, and the queue function no longer accepts one. Its test asserts a message *arrived*, and was proved by deliberately breaking the ids. |
| 2026-09-01 | **D128 — the workflow's own rewriter writes the tags, and Alexia writes plain English** | `Photo Reference` sends its plain-English box through an `OllamaGenerateV2` node with a danbooru system prompt its author tuned, and Alexia has a model of its own — so both could write the prompt, and **doing both is the failure**: the model writes one, the graph rewrites it, and a bad result has two places to have come from. The graph keeps it. A workflow's own vocabulary surviving is half of what this was built for, and the field is named after the author's own title, which is what tells the model to write a sentence rather than a tag list. |
| 2026-09-01 | **D127 — the Model dropdown was a promise the contract cannot keep** | `plugins/media` shipped a `choice` widget offering only `auto`, under a hint saying the list fills in once it can reach ComfyUI — on a machine with six checkpoints installed. It cannot: `choice` options are fixed in the manifest, and core refuses any plugin write to a widget that is not a `status`, deliberately, because a plugin that could rewrite a toggle could quietly undo a decision the person made. It is a text box now, matched loosely, so typing `hassaku` finds the file nobody wants to type; `auto` keeps its old meaning because it is what every existing install has saved. And the state line names a model that is not installed, because otherwise a picture still comes out, painted by a different model, looking exactly like the plugin working. |
| 2026-09-01 | **D126 — a stale export is refused, and the refusal names the menu click** | Consuming an export means a workflow edited and not re-exported leaves Alexia running the older graph, and what comes back is a **picture rather than an error** — so nothing downstream catches it and every later picture is indistinguishable from a right one. The habit here is the honest sentence over the refusal; this is the case that habit is weakest against, so the refusal wins. ComfyUI's own file listing carries both modified times, so *which is newer* is arithmetic. Running the old one on purpose stays possible and has to be asked for. |
| 2026-09-01 | **D125 — a node its author titled is a node they meant** | How a workflow says *this is a knob*. The proposal was an `alexia:` prefix; the evidence chose the friendlier reading. Sixteen of twenty nodes in `Photo Reference (Pose + Style)` keep their default title and the four that do not are exactly the knobs — and `Simple SDXL + LoRA` carries three of the same titles word for word. The prefix would have meant re-titling every knob of every workflow first. The trap: **every** node exports a title, an untitled one carrying the class's display name, so presence proves nothing and the test is *different from what it would have been*. Knobs land in the tool schema, never the settings pane. |
| 2026-09-01 | **D124 — one plugin, not two** | Whether a workflow runner is a new `comfyui` plugin beside `media`. It is not, and the reason is in core: a capability is routed **by name only**, first enabled provider wins, and the result never says who answered — so two plugins declaring `image.generate` would hand Combined mode whichever iterated first. Across thirteen plugins no capability has two providers, which is load-bearing. And the guard against starting two ComfyUIs is a pid in per-plugin storage, which a second plugin cannot see — two of them on one graphics card being the worst end available here. The real argument for splitting is that *Local image generation* does not describe something that runs audio; that is a naming problem, and it waits. |
| 2026-09-01 | **D123 — ComfyUI's API export is the contract** | What the editor saves and what ComfyUI's own queue accepts are two JSON formats that look alike, and **nothing converts between them over HTTP** — the conversion lives in its frontend. Re-implementing it fails on exactly the graphs it would be built for: three of the five obstacles are in the workflows saved on this machine, including a loader storing objects where widget order says scalars and one whose widget count is driven by another widget. A converter that handles the plain cases and fails on the interesting ones is worse than none. So the frontend does it, once per workflow, from a menu entry — and the cost of that is D126. |
| 2026-09-01 | **D122 — a file a task made reaches a phone, not only the window** | `document_plan_final.md` §8 gave a tool a way to hand a file back — a `resource_link` block, a row under the answer to open or save it, the model told only `[file: name]`. That row fetches from a `/api/file` route, and a **channel plugin is in its own process** with no token and no way to reach it, so asking for a picture over Telegram was still a sentence with a path in it — which is exactly the transcript that prompted this. Core now reads the bytes for a plugin-started task and returns them base64 on the sampling result's `_meta`, under a key an older Alexia ignores, with the same size ceilings an upload has. Telegram sends a raster image as a photo and anything else as a document, a rejected photo falling through to a document; the `FormData` upload its own file comment has predicted since M7-5 finally landed. The picture plugin's text also stopped naming the save path — the file is a row to press now, and a model reading a path back to somebody who cannot open it is the failure D117's transcript showed. The window is untouched: it takes the files off the step trace and never needed the `_meta`. |
| 2026-09-01 | **D121 — an About page, because *which version am I running* had no answer on screen** | Asked for from use, and the gap was real: the number was in the binary, in the release notes, and in an error message about a plugin needing a newer Alexia — nowhere a person would look. So a third settings tab holds the version, who made it, the two licences and why they differ, and the update controls. **The update controls are three things and are said as three things.** *Check now* asks GitHub this second, for somebody who has heard there is a release and should not have to restart to find out. *Update now* appears only when there is one. And a switch governs **looking**, not installing — which is the wording the whole page turns on, because *auto-update* is a phrase people reasonably read as *it will change under me*, and nothing here has ever installed itself without a press. Turning it off is a supported answer rather than a mistake: staying on a version you trust is a decision, and the sentence under the switch says exactly what stops happening. The default stays *look*, because an assistant that quietly stops updating is one running last month's bugs on purpose. **The version comes from `/api/state`, not from the shelf** — the network being down is exactly when somebody asks what they are running — and the preference is stored beside the theme, so a window and a tab pointed at one core cannot disagree about it. |
| 2026-09-01 | **D120 — what is not installed goes in the grid, dimmed, because the disclosure was a good hiding place** | Reported from use, one hour after D118 shipped: *the plugin page doesn't give me a way to install it back — I don't see any part of the UI where non-installed plugins live.* They lived in a collapsed `details` under the grid, put there on an argument that was sound in itself — this screen is where somebody comes to change something they **have**, and a page opening on forty things they do not is a shop rather than a settings page. The argument was about a shelf of forty. The shelf has eleven, and the first person to delete a plugin and want it back could not find where plugins come from, which is **the one journey that starts on this screen and cannot be finished anywhere else**. So both halves share one grid: installed first, then a labelled row, then the rest dimmed. **Dimmed rather than greyed out**, and the distinction is the whole of the design: a greyed control is one you cannot use, and these are the only cards on the screen that exist *to* be used — so the edge goes dashed and the ground lighter, while the text keeps full contrast because it is what somebody reads to decide, and hovering brings the card all the way up. The label row is not decoration either: a paler card with nothing explaining it reads as broken rather than as available. The forty-plugin worry is answered by the filter box that was already there. |
