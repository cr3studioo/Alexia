...
- Core contributes only the tabs whose data core owns; **every other tab is declared by the
  plugin that owns it**
- `table` is the eleventh widget (D83); every state-changing route is guarded or declared
  safe (D82); a skill a model wrote waits for a yes (D84); a plugin declares its tab in its
  manifest, at `alexia_protocol` 3 (D86)
