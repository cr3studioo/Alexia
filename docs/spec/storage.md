...
| secrets | OS keychain, account `voice.<key>` — service `dev.alexia.app` held by the desktop shell, `alexia` when core runs on its own (D153) | one delete per `password` setting |
