...
| secrets | OS keychain, account `voice.<key>` — under the desktop app a key inside the one entry `_core.vault` of service `dev.alexia.app`, held by the shell (D153, D187); service `alexia` when core runs on its own | one delete per `password` setting |
