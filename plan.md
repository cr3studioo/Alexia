...
| Date | Entry |
|---|---|
| 2026-09-21 | **D186** — **today's allowance holds each paid rung to its worst case.** *(Draft.)* `route()` puts what today has left on the verdict where the allowance let paid in; `send()` skips a paid rung whose `dearest()` — everything sent at `PER_TOKEN`, plus the whole reply ceiling — is more than that, and goes on to something cheaper or free. Chosen over a fixed token ceiling for presses, which caps tokens rather than money. *Paid only*, pins and the monthly hard stop are unchanged. |
| 2026-09-21 | **D185** — **the personality's length is chosen per model asked.** *(Draft.)* Replaces D181's *weakest rung in the plan*: an Automatic plan's tail is nearly always a router or a 2B, so the paid model at its head was sent the short one. `send()` asks for each rung's messages as it asks it (`messagesFor`); the trace records each rung actually asked (`onAsk`); `weakest()` is gone. `sizeFor()` and the plugin contract are unchanged. **M8-7**. |
