...
- [x] **M9-2** The catalogue and its search (`catalog.js`, `find_workflow` — D133), **installing one** (`install_workflow` — D137), **steering it** (`wired()` — D138), **adding one from a link or a file** (`add_workflow` — D139), and the page that lists and removes them (`workflows`, `remove_workflow`). **C2 was not needed** — `table` is already *a list of things with actions on each one* (D83); C4’s drop zone is convenience over a path that already works
