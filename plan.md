...
- [x] **M8-6** Models: listed when usable, ranked best to worst, and three fallback modes *(inserted 2026-09-15, D154–D155, D158–D161; built out 2026-09-18, D172–D174 — see [`model_plan.md`](./model_plan.md))*
- [x] **M8-7** Personality v2: three sizes and ten improvements *(inserted 2026-09-15, D156–D157, D160–D161; built out 2026-09-18/19, D175–D177, D179–D184 — see [`plan-personality.md`](./plan-personality.md))*

### M9 — Local media generation, as a product *(inserted 2026-09-01 — see [`comfyui_engine_plan.md`](./comfyui_engine_plan.md))*

Twenty-four owner answers and an approved plan. **Roughly sixty per cent of this is core, not
plugin** — a thirteenth widget, a recursive sub-library, richer trace entries, one new lifecycle
event. Sequenced so nothing waits on something later.

- [~] **M9-0a** A **verifying converter** instead (D134) — measured at 6 of 45 templates, refusing the rest by name. The hidden webview is still what would get the other 39, and is not built
- [x] **M9-0b** Websocket progress — per-step, matched `client_id`, and a test that a message *arrived*
- [x] **M9-1a** Renamed *Local media generation*; id and namespace stay `media`
...
- [x] **M9-2** The catalogue and its search (`catalog.js`, `find_workflow` — D133), **installing one** (`install_workflow` — D137), **steering it** (`wired()` — D138), **adding one from a link or a file** (`add_workflow` — D139), **a page to browse and install from** (`library`, `about_workflow` — D140, drawn as cards by D141, the Templates tier only), and the page that lists and removes them (`workflows`, `remove_workflow`). **C2 was not needed** — `table` is already *a list of things with actions on each one* (D83); C4’s drop zone is convenience over a path that already works
...
### M10 — The board *(inserted 2026-09-24, D199 — in progress)*
...
- [~] **M10-6** `page` in the manifest at `alexia_protocol` 11, four plugin pages, and the Vtuber placeholder in the registry
...
**Revised 2026-08-31 (D117): it starts ComfyUI too.** *Not running* was this plugin's
commonest state and its answer was to hand the user a chore. It now finds the install, starts
it with the install's own Python, and leaves it running after the plugin itself is stopped —
`proc.spawn`, named in the `why`, and never killing a ComfyUI it did not start. Two things
that only turn up by making a picture came with it: the venv is the only Python with PyTorch
in it, and the fp16 fix's `VAEDecodeTiled` had gained three required inputs, so the default
path was refused by every current ComfyUI and the refusal's body — which says exactly that —
was being discarded.
...
**Fixed 2026-08-29 (D102): quitting left the core running.** Found by looking at Task Manager
on a real desktop, which is the only place it is visible — two `alexia-core.exe` alive with
dead parents, each holding a port and the database open. `spawn()` hands back a handle and
**dropping it does not stop the process**, so the tray's Quit ended the window and left the
daemon; the next launch then put a second core beside it on the same SQLite file. Two halves,
because neither reaches the other's case: the shell kills the core on `RunEvent::Exit`, and
the core watches its own parent and exits when it goes — which is the only one that survives a
crash, an abort (the release profile is `panic = "abort"`) or Task Manager. **Closing a window
still only puts Alexia away**; that is the daemon and it is untouched, and the check holds
`prevent_close` in place beside the rest so a later tidy-up cannot take the tray with it.
`packages/core/test/sidecar.test.ts`, and **not an eleventh invariant** for M6-1's reason.
...
*Rebuilt 2026-08-31 (D118, D119). What this said — "two update paths that share most of their
code: first-party plugins ride along with the app" — is what changed.*

**One update path, because nothing rides along any more.** No plugin ships inside the
installer, so every plugin on the machine, ours included, updates from the same shelf: the
GitHub Releases of this repository, read in one API call, cached fifteen minutes, compared
against what is installed. A newer version this build cannot run is *reported* rather than
offered — it is the reason to update Alexia, so hiding it would hide the reason.

**Two questions, not one.** `alexia_protocol` is the shape of the contract and `min_app` is
the build, and only the second one catches a plugin that needs a capability added last week.
`versionVerdict` answers both, and it is asked in three places — the shelf, the install, the
loader — so nothing is offered that cannot install and nothing installs that cannot load.

**And the app itself**, which is the half that makes the floor real: `tauri-plugin-updater`
against a signed `latest.json` on the app's own release. One check at startup, a strip rather
than a dialog, and `installMode: "quiet"` — `/S /R`, silent then restart. The plugin exits the
process when the installer launches, so there is no success branch in the page; the failure
branch is the only UI, and it has a manual link behind it.
...
is **G13**, open in `questions.md`, and this task does not pre-empt it. *G13 was answered
2026-09-15 (D156): a button somebody pressed is a run, and may spend like one.*
...
- **Two corrections, found by probing this Mac's shape before trusting any of it.** Neither is
  a refinement; each was the change making the thing it was built to fix worse.
  1. ***Best-first* is not *exactly what `prefer: 'best'` does*, and this text said it was.**
     `/best` turns §8.2's rungs round along with the money — a leftover of the comparator
     having one `money` boolean — so a request for a model that can write chose one with no
     tools, and then the keyless floor's 7B ahead of a 550B model on the owner's own key. The
     keys now name an **axis**: `price` (free/paid, tier, cost), `reach` (tools, whose key it
     is on) and `sure` (what failed here, router, size, usage). `/best` turns `price` and
     `reach` exactly as it always has; a capable request turns `price` alone; nothing turns
     `sure`. Among free models the price axis ties, so a free-only install gets the same order
     it always did — there, it is the two filters that earn their keep.
  2. **`min_tier: "T1"` killed the personality plugin in Local mode.** Honoured, it means
     *never a model on this machine* — and in Local mode the pool **is** this machine, so
     Adapt became a button that cannot be pressed for exactly the people who chose Alexia for
     privacy. `plugins/persona` declares `T0` now and asks for a capable model by preference
     instead; `manifest.md` warns the next author, because a field that was inert for a year
     is a field nobody's declaration was ever tested against.
...
`store.sessions()[0]` into a `const` at startup and every append went there — one transcript
growing until `trim()` was the only thing between it and the context window, with no way to
start a fresh one or to find something said last week.

Everything needed was already in the store: `createSession`, `sessions`, `deleteSession`,
`history(id)`, and `ON DELETE CASCADE` on the messages. What was missing was core using more
than one of them, and a screen.

**The screen is a `table`, which is the whole point.** Chats is a core tab like Activity or
Models — a declaration and two functions, drawn by the same widget code that draws a
plugin's. It needed one thing the other five did not, a *New chat* button that is not about
a row, and `action` is one of the eleven widgets, so that cost nothing either.

Three rules the tests hold:

- **The open conversation cannot be deleted.** Its messages go by cascade, so every later
  append would point at a session row that is not there.
- **An empty conversation is reused rather than stacked**, because pressing New twice means
  the same thing as pressing it once.
- **A task already running finishes into the conversation it started in.** The switch takes
  effect on the next thing said, which is what the button says it does.

Titles are the first thing the user said, read out of the messages table in the same query
that lists the conversations. Not a model-written summary: *the one where I asked about the
printer* is already on disk in their own words, and a second name is a second thing that can
be wrong.

**The M6-4 assertion moved, deliberately.** It read *a core tab holds tables and nothing
else*, which is stricter than the property its own next line names. It is now the manifest
schema itself — core's widgets are parsed as a plugin's `settings` — so *what a plugin may
...
- **Run against the live ComfyUI on this machine, and it made a picture.** ✅ `/object_info`
  (2,173 classes), `/userdata` listing, `POST` and `GET` of a filename carrying spaces and
  brackets, `/prompt`, `/history`, `/view`, and the file deleted afterwards. The binding was run
  against **the real `Photo Reference` and the real `/object_info`** rather than the fixture, and
  bound exactly `load_model`, `plain_english`, `fixed_tags` — with the untitled sampler and the
  titled display both correctly left out.
- **What is still untested is ComfyUI's own *Export (API)* menu.** The one rule it turns on was
  confirmed in the frontend bundle — `_meta:{title:e.title}`, which litegraph answers with the
  custom title where there is one and the class's display title where there is not — and the
  titles were reproduced from that rule rather than exported. Running the real exporter on a real
  workflow is the test that has not happened.
...
is the whole of *they are there but not working*. The owner approved a prototype that replaces
them with a board, and answered the four questions it raised: Chat is a page and can be removed;
there is one free layout and the columns are guides inside it; the *tabs in the middle* were the
grips; General is never removed.

### M10-1 `layout.ts`

`packages/ui/src/layout.ts`, and nothing in it touches the DOM — the precedent is `force.ts`, and
the reason is the same: arithmetic is tested without a browser. `grid(W, H)` puts dots every
25 px with at least 20 px of margin and splits the spare pixels evenly each side. `pack` places
the pages a person put somewhere first, then the rest at the first free spot, one clear dot
between any two. `dragGuide` moves every page edge sitting on a guide, and no further than the
smallest and largest of those pages allow. `rescale` scales a layout to a new width and packs it
again. Under 28 dot spaces the pages stack in one column and the saved layout is not touched.

**Acceptance.** Centred at 1337 px and at 520 px; the one-dot gap holds; a guide stops at a
page's minimum; rescaling and back lands where it started; compact mode leaves `layout` alone.

### M10-2 The board, core pages only

`#shell` becomes `#board`. General (the rail, `mountRail` reused), Chat (`#chat`, unchanged),
Running now, Steps and Current step (from `live.ts`) and Price are pages, each mounted into a
root it is handed rather than finding `#id` for itself, and each on the board once, so no id is
ever duplicated. Settings and Activity open as a glass sheet over the board; Esc closes it. A
page moves by `transform` while dragged, with its blur off, and writes its position when the
drag ends — thirty `backdrop-filter`s repainting per pointer move is the stutter otherwise.

**Acceptance.** Both grips drag and the pages on them follow. General shows no remove.

### M10-3 Keeping the layout

`{ v: 1, cols, guides, pages: [{ id, w, h, anchor }] }` in core's kv as `layout`, written through
`/api/setup` and returned in `/api/state`, the way the theme is. A copy in `localStorage`
(`alexia.layout`) is read by the pre-paint script, so the layout does not flash on launch.

**Acceptance.** Relaunch: the layout is back, and there is no frame where it was not.

### M10-4 Edit view

A 48 × 48 px corner, bottom left; after 150 ms a pill slides out with **Edit view**. Tab reaches
it, the palette has *Edit layout*, a long press on the empty board does it on touch. In Edit view
the dots show; a page snaps to dots, blue where it can land and red where it cannot, and goes
back if dropped on red. The pill becomes **Add page · Reset · Done**. Removing Chat with no other
channel connected asks once, and *Add page* can always bring it back.

### M10-5 Local stats

A new core page and `/api/local-stats`: the Ollama models installed, which are loaded
(`/api/ps`), their size, and the tokens a second they last ran at. Built on `ollama.ts`.

### M10-6 Plugin pages

`page` in the manifest, `alexia_protocol` 10 → 11, MIN stays 2. Validated in
`packages/protocol`, in `plugin.schema.json`, and in conformance: tiers do not shrink, tiers sit
inside `scale`, `fixed` has one tier, `show` names only declared keys, and `page` at 10 is a load
error. A plugin with a `panel` and no `page` gets a default M page from it. Memory (graph),
persona, voice and media declare one. The Vtuber model is a registry entry with
`coming_soon: true`, greyed in *Add page* and not installable — core never names it.

**Acceptance.** Install `voice`: *Voice page added*, at the first free spot. Disable it: hidden,
spot kept. Delete the folder: gone, and `layout` holds no id for it. Crash it: the page shows
*Restart*.

### M10-7 The docs

[`docs/authoring/pages.md`](./docs/authoring/pages.md), [`manifest.md#page`](./docs/spec/manifest.md#page),
a paragraph in `ui-schema.md`, *10 → 11* in `versions.md`, and D199. Done 2026-09-24.
...
1. **Linux** — a port, not a rewrite. macOS was the same claim and held (D144–D148): core needed nothing, and the work was the packaging, the shell and the three plugins that were Windows code.
...
| **M5-4+** | Put the minisign private key from `~/.alexia/updater.key` into `TAURI_SIGNING_PRIVATE_KEY`. The release workflow refuses to publish without it rather than shipping an installer every existing Alexia would then refuse. No password secret is needed — the key was generated without one. The public half is already in `tauri.conf.json`, and it is **not** the code-signing certificate: a release can ship unsigned by Authenticode and still update people correctly |
...
| 2026-09-21 | **The keychain migration, run on a real keychain.** Not a decision — a fact checked. `pnpm check:keychain` builds `vault.rs` under the service `dev.alexia.app.migration-test` and moves test keys through this Mac's login keychain with core's own `fromShell()`: an old `alexia` entry and a D153 per-account one move into the one entry and leave their old places empty, a key planted in the old place afterwards is not adopted, saves and clears land, five reads at once answer, a client dribbling a byte a second is cut off at five seconds without holding up a request behind it, and everything written is deleted. Passed on 2026-09-21. |
| 2026-09-21 | **D188** — **the PR #22 review's fifteen.** `/best` turns only `price`; background asks free-and-unrationed, then a rationed day's first half, then paid; *Bad answer*'s `above` falls back to any tier; the pause carries the real stop sentence and a plan nothing could reach never pauses; `limitsPerModel` for Groq and Cerebras; a `max_tokens` refusal is the model's; billed empty replies are recorded; `byProvider` keyless set-asides are still tested daily; `Message.typed` and `Message.row`; core bugs in `chat()` thrown as themselves; one run at a time in the shell; *Use this* on a local model; the personality check's contractions and style lines; Undo keeps its notes; memory's newest-first ties. |
| 2026-09-21 | **D187** — **the desktop keychain as one entry, older entries moved in once.** `kept()` holds every secret in `_core.vault`, read once and serialised, so an update asks the keychain once rather than per secret. Older homes (D153's per-account entries, core's `alexia` ones) are looked in once per account and recorded (`looked`): copied first, deleted second, a failed delete said and retried next launch, a refusal asked again next launch. The build check's stand-in keeps what it is given and seeds `moved: 'all'` — it had deleted a developer's real keys. The shell opens the vault before spawning core, reads each connection on its own thread with a five-second whole-request deadline, and passes core an allow list of environment variables. Purge carries on past a refusing keychain. |
| 2026-09-21 | **D186** — **today's allowance holds each paid rung to its worst case.** `route()` puts what today has left on the verdict where the allowance let paid in; `send()` skips a paid rung whose `dearest()` — everything sent at `PER_TOKEN`, plus the whole reply ceiling — is more than that, and goes on to something cheaper or free. Chosen over a fixed token ceiling for presses, which caps tokens rather than money. *Paid only*, pins and the monthly hard stop are unchanged. |
| 2026-09-21 | **D185** — **the personality's length is chosen per model asked.** Replaces D181's *weakest rung in the plan*: an Automatic plan's tail is nearly always a router or a 2B, so the paid model at its head was sent the short one. `send()` asks for each rung's messages as it asks it (`messagesFor`); the trace records each rung actually asked (`onAsk`); `weakest()` is gone. `sizeFor()` and the plugin contract are unchanged. **M8-7**. |
| 2026-09-19 | **A try is stamped by the clock its caller reasons in.** Not a decision — a fact made true. `send()` wrote every try to the model record with `Date.now()` whatever the caller thought the time was, so `trial.ts`, which takes its own `now` and reasons in days, wrote tests into one day and read them back as another. That is what made `trial.test.ts` fail on 2026-09-18 with nothing changed (`61e33b8` anchored the fixture and left the seam). `send()` takes an optional `at`; absent is the wall clock, which is every other caller. `56549d1`. |
| 2026-09-19 | **D184** — ***That wasn't her*, in one row with *Bad answer*, and the chip.** Improvement 10 and improvement 8's chip, built together as item 17 required — separately they are two rows competing under one bubble. The two ask opposite questions: *Bad answer* is about the model and re-asks; this is about the personality and re-asks nothing, leaving the answer on the page marked rather than dimmed. One press sends the fact, the optional line turns it into an example, and a new capability `persona.not_her` carries it to whoever keeps it — core forgets it and never fails the press on a plugin. Refine reads the last four, its own row's, after the document and as examples. The chip is `persona.in_use`, a second capability because the two are read at completely different rates, hidden when nothing is chosen. Invariant 1 caught the chip's first name, `#persona`; it is `#character`. `b0f5db5`. **M8-7**, `plan-personality.md` improvement 10. |
| 2026-09-19 | **D183** — **facts to memory on one press, and a personality bound to a place.** Improvements 5 and 9. Facts come out of the same Adapt call as the three lengths, only when `alexia/answers` says something will remember them, and are **offered rather than taken** — one button for all of them, which is D160's *one confirm* in the only shape the widget set can draw (a plugin writes only its own `status` settings, so a dynamic tick-list is not one of them). Core passes `{ channel }` to `persona.personality`, naming the plugin that started the task; optional at both ends, nothing sent for the window, so no contract number moved. A bound row is not a second kind of *in use* and neither action touches the other flag. `0d1887d`. **M8-7**, `plan-personality.md` improvements 5 and 9. |
| 2026-09-19 | **D182** — **lines she cannot act on, and `alexia_protocol` 10.** Improvement 4, and the seventh `alexia/*` method — the first since 2026-08-28, argued as `wire-protocol.md` requires: a capability is Alexia's own idea and MCP has no cross-server *does anybody offer X*. `alexia/answers` returns two booleans, names nobody at either end, and does not need the capability in `requires[]` because asking runs nothing. No manifest field moved, so the schema is untouched. The owner chose the plan's literal method over a broader ability list with the trade stated — only capability names can be asked about, so a line naming nothing a capability covers cannot be checked, and the note says how many lines were **looked at** rather than implying it looked at all of them. Deterministic whole-word matching, on save and again when a row is opened; it flags and never removes. Found on the way: `memory.remember` and `memory.recall` were shipped and never registered in `capabilities.md`. `2e5ed4b`. **M8-7**, `plan-personality.md` improvement 4. |
| 2026-09-19 | **D181** — **three lengths of a personality, chosen for the weakest rung in a step's plan.** §2, and the reason it matters: a personality is in the system prompt on every step, so six hundred words across a fifteen-step task is 7–8k tokens re-sent. Core's `sizeFor()` keeps the section's own rule — *when in doubt, the weaker reader*: small for under 7B, a window under 32k, any router, or an unpublished size on a model the record doubts; high for a paid model; medium for the rest. `weakest()` picks for the whole plan, because a 429 hands the step to the second rung. What is reported is what was *sent* rather than what was deserved, so a one-document personality reads as `high`. **This narrows D175**: the trace line moves per step, because the three lengths are exactly the thing D175 said could not change under a task — repeats collapse, so one length across fifteen steps still reads as one fact. The plugin writes all three in one call on two markers, drops rather than trims a shorter one that ran long (§2's instruction — the hard rules are at the end), raises `ROOM` to 6,000, and clears the shorter two on Edit. `structuredContent` carries the three; `text` is still the long one, so no contract moved. `093f9c4`. **M8-7**, `plan-personality.md` §2. |
| 2026-09-19 | **D180** — **hearing her before she goes live: Adapt stops switching, and the Skip is a toggle.** Adapt saved and switched in one press, so the first anybody heard of a new personality was a real conversation in it. It now saves the row **not in use** and asks her two questions; **Use** changes how she behaves and **Forget** throws it away. Saved-but-not-in-use rather than the plan's unsaved draft, chosen by the owner from three: the plugin is `lazy`, so a draft held between two presses is sometimes gone by the time somebody decides. The Skip is one toggle, on by default, and off is exactly what Adapt always did; the checks run either way. The questions are *who are you?* and a plain moment built from her own *What you do without being asked*, with the line it watches for quoted — never a scene a model invented, which is a third call and puts made-up facts about this person on screen. Answered with no `modelPreferences`, so it is the model the chat would use. Refine and Edit do not run it; **Hear her** is a row action for any time. `22d491b`. **M8-7**, `plan-personality.md` improvement 3. |
| 2026-09-19 | **D179** — **Refine and Edit: a sentence rather than a rewrite, and the change shown line by line.** Brought forward ahead of the preview on the owner's choice. Refine sends the document and one sentence instead of the 1,300-token description that ran a reasoning model out of room (D157), through the same helper and therefore the same clamps; Edit is a `multiline` box for changing the words by hand, through both checks, recorded as written by *you*. It cannot be prefilled — a plugin writes only its own `status` settings and core offers no `elicitation` — so the hint says to copy from the row. The diff is an exact LCS over lines with no dependency, after the save with Undo as the way back. It uncovered a shell bug: a tool's answer renders into a `<p class="hint">` and every newline collapsed, so Adapt's Markdown document has always arrived as one paragraph; `.said-lines` fixes it at the four places a tool's words land. Manifest declares `alexia_protocol` 7 for `multiline`; no contract field moved. `887a7c8`. **M8-7**, `plan-personality.md` improvement 2. |
| 2026-09-19 | **D178** — **`modelPreferences` and `min_tier` are honoured, a router pin is stepped around, and a press spends like a run.** M8-1, and `plan-personality.md` §1. One of MCP's three priorities reaches the router — `intelligencePriority`, largest of the three and at least a half — and the other two are documented as read-and-not-acted-on rather than left to be measured: nothing core knows says how fast a model answers, and a `hints` entry is a plugin choosing a vendor. It becomes `Ask.capable`: best-first, **never a router** (a filter, because a different model each time is not an answer to *give me one that can write*), and never a model the record doubts while anything else fits (D161's reading of *set aside*). A pin still wins outright with one owner-chosen exception — a pin on a router is not a pin on a model — which is the one point where this plan's *a preference is not a way past a pin* and `plan-personality.md`'s *even when the chat pin is a router* disagreed. `min_tier` comes off the manifest on both sampling paths, and a floor nothing reaches now names itself. G13/D156 is built: a request made while the plugin's own press is in flight carries a run id, so it may reach paid under the switch, the day's amount and the monthly cap; without a press, G12/D96 is unchanged. Two corrections came out of probing this Mac's own catalog before trusting it: *best-first* is **not** *exactly what `/best` does* — `/best` also turns §8.2's rungs round, which chose a model with no tools and the keyless floor's 7B over a 550B on the owner's key — so the ranking's keys now name an axis (price · reach · sure) and a capable request turns price alone; and `plugins/persona`'s `min_tier: "T1"`, honoured, killed Adapt in Local mode, so it declares `T0` and asks by preference. No manifest field moved, so no protocol bump. `9a1d745`, `dc227b9`. **M8-1**, **M8-7**, `plan-personality.md` step 5. |
...
| 2026-09-18 | **D174** — **the rest of the Models mock-up, and `alexia_protocol` 9.** The group line and the filter chips (*Needs attention* / *New* / *Set aside*) are declaration fields, not row data: `table` is the plugin contract's and its schema is `.strict()`, so they cannot be smuggled in on a row — the same shape of argument as `groupOrder` in D164. `ALEXIA_PROTOCOL_MAX` 8→9, floor stays 2, schema regenerated, documented in `manifest.md`, `ui-schema.md` and `versions.md`. Row-opens-on-click needed nothing at all: pure shell behaviour on a table that already declared `detail`. `b0f6f21`. **M8-6**, `model_plan.md` §4 C. |
| 2026-09-18 | **D173** — **the keyless group's on/off switch, read where the rungs are built.** §1's last open piece (D154 settled that it exists). The switch is read in `usable()` rather than only where the tab is drawn, so the rungs are not built at all when it is off and `route()` cannot disagree with the Models tab. Off behaves exactly as removing a key does (D163): out of every plan, listed entries kept and marked *not available*. `cb53095`. **M8-6**, `model_plan.md` §1 step 2. |
| 2026-09-18 | **D172** — **a set-aside sentence promises a test only where one can reach.** The sentences for *answers empty*, *always busy for you* and the timeout case ended *One good reply brings it back.* with no mention of a test, written when §4 E did not exist. §4 E is built (D166), but `due()` sends tests to free models only and never walks `world.local`, so promising one on every set-aside row would have replaced one untruth with another: `setAside()` now takes whether a test can reach the row. Found beside it: a paid model refused all day never reaches that sentence anyway — it stays in *Paid*, because the set-aside group only takes rows no plan above it showed. `5855c62`. **M8-6**, `model_plan.md` §4 C, §4 E. |
| 2026-09-17 | **D171** — **`model_plan.md` §4 J as built: the hook for later sharing.** `store.report(since)` → `Reported[]`: `week` (UTC Monday, `YYYY-MM-DD`), `provider`, `model`, `tries`, `answers`, `failures` by `Outcome`, `badAnswers` — no prompts, answers, keys, names, sources, statuses or times of day; a press is not a try. `World.reported?: ReadonlySet<string>`, `serve()` passes an empty set; `judge()` does not read it yet. `report.test.ts` holds the fields and fails if anything in `packages/core/src` or `packages/ui/src` calls `.report(`. **M8-6**, `model_plan.md` §4 J. |
| 2026-09-17 | **D170** — **`model_plan.md` §4 I as built: *Bad answer*.** `POST /api/chat { again: true, bad: {} }`: `store.markLastAnswerBad()` marks the latest answer (`Message.bad`, kept on the page, filtered from everything a model is sent; 409 when there is none), a `bad-answer` try from `person` for the model and provider stored on it (`Message.provider`, new), and the question asked again on Automatic with `Ask.avoid` and, with `caps.cross`, `above` its tier. The shell's row of message actions under the latest answer, where *that wasn't her* (`plan-personality.md` improvement 10) joins later. **M8-6**, `model_plan.md` §4 I. |
| 2026-09-17 | **D169** — **`model_plan.md` §4 H as built: crossing into paid.** `caps.cross` under the slider (on asks for `caps.daily`); `World.cross` = switch on or *Allow* pressed in the conversation; off, `route()` and the loop end `paused` (nothing billed) with a reason, and *Allow* (with an amount box at $0) covers the conversation and carries on; on, paid goes ahead of this Mac and the day's amount stops it with a sentence saying so; the *slow local or paid* question removed; a phone task asks on the phone and stops after ten minutes. Found: `/api/chat` kept the running task after a refusal (released in a `finally` now). **M8-6**, `model_plan.md` §4 H. |
| 2026-09-17 | **D168** — **`model_plan.md` §4 F as built: the chat first on free requests.** `Ask.background` for a plugin's task, a plugin's sampling request and a daily test; `route()` gives background providers with no daily ration and this Mac first, then day-limited ones only while more than half their day (or month) is left (`underHalf()`, ledger or header, whichever is lower); pins and lists held to the same half; the stop says the rest is kept for the chat. Adapt counts as the chat by its press (a request during its own button press), until G13's run id. **M8-6**, `model_plan.md` §4 F. |
| 2026-09-17 | **D167** — **`model_plan.md` §4 G as built: a switch said twice.** `send()` reports a switch as an event (`onSwitch`: from, to, reasons, sentence) and the charge line through `onPaid`; the loop keeps switch sentences on the answer as `notes`, never sent to a model; `/api/chat` streams `switch` and `paid`; the shell shows a three-second pop-up, the line above the answer (drawn again after a reload), and the charge line in its own place above the message box. A switch into paid is now said as a switch too. **M8-6**, `model_plan.md` §4 G. |
| 2026-09-17 | **D166** — **`model_plan.md` §4 E as built: the daily test message.** *Reply with the single word OK.* to new and set-aside free models (not *needs a key* while no key is saved), at most ten a day, oldest evidence first; a round a minute after start and on every six-hour tick, the day's budget kept in the store; never while a task or a plugin's request is being answered; recorded as `test`, never in a conversation or the spend ledger. Still open: each free tier's terms, before a public release. **M8-6**, `model_plan.md` §4 E. |
| 2026-09-17 | **D165** — **`model_plan.md` §4 D as built: keeping it current.** A six-hour poll in `serve()`; `added` per provider (it was counted across providers) and `listKnown`, written to `seen`; OpenRouter's `created` and retirement dates read where a row says its list carries them (`PARSER` 7); a news line above the Models table, said once per provider fetch; rate-limit headers (Groq's day, OVHcloud's minute, OpenRouter's on a 429, `retry-after`) kept in memory and only ever lowering the ledger, `retry-after` a *busy* tag; OpenRouter's key endpoint (`keyInfo`, `rpdFunded`) read on save and every tick, giving the real day and §1's *Funded* — paid models left out where the account cannot pay. Four rows checked 2026-09-17: Groq per model (1,000 a day), OVHcloud still 2 a minute, LLM7 wants a key for some models only, **Cerebras's trial wants a card** (owner: keep the row, say so on the tile, key wall works it out from the rows). Owner's answer, changing D161/D162: a keyless provider is set aside whole only when none of its models answered; otherwise *needs a key* is per model. **M8-6**, `model_plan.md` §4 D. |
| 2026-09-17 | **D164** — **`model_plan.md` §4 C as built: the Models table.** Rows are `route()`'s plans in five groups — *Your choice* (a pin; the plan had no place for one), *Your list*, *Automatic, free*, *Set aside by Alexia*, *Paid* — one per model per provider, with `explain()` on the row above as the sentence under each, size, abilities, the world's usage and whose it is, answers in 30 days, price and `judge()`'s tags. Owner's answer: the `table` widget is the plugin contract's, so **`alexia_protocol` 8** adds `groupOrder`, and a row's `note` and `tags`, for every plugin (floor stays 2). On this Mac's catalog copy the Automatic group equals `route()` row for row (103 rows, 125 with an OpenRouter key) and every why-line equals `explain()`. Found: the usage column read `391965.2M`; the Control view was too narrow (now 76rem). Not built from the mock-up: filter chips, group descriptions, click-to-open. **M8-6**, `model_plan.md` §4 C. |
| 2026-09-17 | **D163** — **`model_plan.md` §1 steps 3–4 as built, and keep-alives capped.** Saving a key waits up to 15 s for that provider's list and answers with what it unlocked under the slider, or why the list did not come; the line shows where the key was saved and the rail (and the Models tab, if on screen) redraws. A key can be removed from settings with two presses; a keyless provider goes back to the floor; a pin or list entry is kept and marked *not available — no key for X*. Found: the ladder dropped list entries it could not find and saved that on the next drag, which would have deleted them on key removal — fixed. Found: Kilo kept a request open with `: KILO PROCESSING` comments for a minute and no answer, and D158 counted those as progress, so a chat could hang for ever; owner's answer, keep-alives count for 120 s at most (`PATIENCE.keptAlive`), then the next model is asked. Changes D158. Not built: *Funded*, the keyless group switch. **M8-6**, `model_plan.md` §1, §3. |
| 2026-09-16 | **D162** — **`model_plan.md` §4 A and B as built.** A plugin's cancel reaches `send()` and a plugin's task (through `host.ts` and `plugins.ts`), checked before every rung, so nothing is asked, counted or recorded after the plugin gives up; the MCP SDK drops a cancel for request id 0, which Adapt never sends and core does not work around. Migration 7: `tries` (every try, 30 days) and `seen`, `strikes` dropped; thirteen outcomes, the plan's nine plus *no credit*, *key refused*, *too long* and *could not be reached*, which never tag a model; strikes are read from the record exactly as D159 struck, so the order did not move. `health.ts` `judge()` with named thresholds; `route()` skips set-aside models after the other filters and asks them when nothing else fits, never refusing a pin; `ranking()` is named keys with `explain()`. Owner's answers: a connection that could not be made sinks and never sets aside; Automatic skips set-aside as D161 says, narrowing D155. Decided in the building: set aside lasts until a good reply, *in a row* means no good reply between, a pin on a model served twice avoids the set-aside copy, the stand-in is the median own figure of its size class, `PLANNER` and `stature()` move to `catalog.ts`. Migration 5 → 7 checked on a copy of this Mac's database. Found: the memory panel test is flaky. **M8-6**, `model_plan.md` §4 A–B. |
| 2026-09-15 | **D161** — **the Models table, designed against a clickable mock-up.** The table is `route()`'s own order in four groups with a why-line per row from the ranking's keys, keyed per model per provider; a 30-day record of every try and first-seen times (migration 7); busy sinks, but a whole day of refusals (3 tries over 2 hours, none answered), 3 empty answers, two 404s or a keyless provider refusing two models sets a model aside, never deleted, back on one good reply (changes D159's *never written off* for that case); a daily test message to new and set-aside models, free, at most 10; a new model starts at the bottom, its first good reply lifts it to its size's middle, OpenRouter's figure then places it; a *Bad answer* button after the table; a refresh every six hours with limits read from answers and OpenRouter's key endpoint; plugin tasks and tests keep off day-limited providers while another can do the job, and take at most half of one otherwise, with Adapt counting as the chat (keeps D156); turning the paid switch on asks for the daily allowance, *Allow* covers the conversation, Telegram asks on Telegram for 10 minutes, the same pause applies when no free model can do the job, and the switch replaces the *slow local or paid* question. Found: the switch line was never saved, the tab's sort was not the ranking, and four provider rows were already wrong. Open: the record sent to the owner's server (hook only), provider terms for automated tests. **M8-6**, `model_plan.md` §4; **M8-7**, `plan-personality.md`. |
| 2026-09-15 | **D160** — **the owner's answers after D159.** Size counts the whole model and only orders; free limits protected per provider account with the watched chat first (not a lock per model); every model switch is a three-second pop-up plus the chat line; a broken-off answer restarts without asking, paid included (D158's length-limit rule unchanged); a switch under the slider for crossing into paid, with a warning under the message box when on and a pause with *Allow switching to a paid model* when off; core stops when Adapt gives up. Personality: about 100 / 300 / 600 words, samples with a Skip button, one yes for all facts, the broken row deleted. Asked for, to be designed with a mock-up first: a Models table that ranks every model with tags (*new*, *busy*, *broken*) and sets aside unhelpful ones on its own, kept current without imports. Decided later: sending model statistics to the owner's server. **M8-6**, `model_plan.md` §4; **M8-7**, `plan-personality.md`. |
| 2026-09-15 | **D159** — **how Automatic ranks, as built.** `model_plan.md` §2 done in core and the Models tab: after free before paid and tools first, what failed on this machine (a strike counts one, halves hourly, forgotten after a day; migration 6), not a router (known by id or name, labelled *a different free model each time*), the ladder, tier and price, a size class read from the id (ordering only, never the planning filter), then `weekly` lent across providers by name. Decided in the building: price stays above size and usage; only failures about the model strike it; a strike reorders and never removes; `/best` reverses only the money half; a key in a keyless provider makes it keyed; a pin on a model served twice picks key first. The size rule reaches local models, so an 8B now also turns the crank beside a 1B. Found: `sampling/createMessage` drops the plugin's cancel, so `send()` walks on after Adapt gives up (not fixed). **M8-6**, `model_plan.md` §2. |
| 2026-09-15 | **D158** — **how the three modes were built.** `model_plan.md` §3 done in core and the shell: failures classified once in `send()` (this model / this provider / this request), `route()` returns `automatic`, `sequence` or `pinned`, patience of 30 s to the first byte and 20 s between chunks for every provider, a dead stream restarts on the next rung with the half-bubble cleared, and a stop in a pin or a list offers *Use Automatic for this answer*. Decided in the building: a paid answer cut off is not bought twice; a keyless 401 is one model wanting a key; a list keeps its money filters and orders one model's providers key-first; the ledger never refuses a pin. Found: four `serve.test` failures were this Mac's Ollama answering, fixed by `ServeOptions.local`. **M8-6**, `model_plan.md` §3. |
| 2026-09-15 | **D157** — **personality v2: three sizes, and ten improvements.** The report was *the personality is not being sent*; what was sent was 221 characters cut off at `## How`, because a reasoning model spent Adapt's 1,200 tokens thinking and nothing downstream could tell a cut answer from a finished one. Fixed first (`maxTokens` passed through, four sections required, 4,000 tokens, 110 s), then the rebuild approved in full. **M8-7**, `plan-personality.md`. |
| 2026-09-15 | **D156** — **G13: a button somebody pressed is a run, and may spend like one.** A call started by a person pressing a declared `action` carries a run id, so the cap and the spend preview apply as they do to a task. D96 unchanged for a plugin on its own clock. |
| 2026-09-15 | **D155** — **three modes, three promises; changes D112.** Automatic falls through the whole ranked list, a sequence only through itself, one model never. Failures classified once in `send()`; a default patience for every provider; a stop names the model and the reason and offers *Use Automatic for this answer*. **M8-6**, `model_plan.md` §3. |
| 2026-09-15 | **D154** — **a model is listed when it can answer, and an unknown price is not free.** One `available()` for the Models tab and the router; keyless providers kept as their own group; key changes redraw the list; Automatic ranked on strikes, routers last, size from the id, and `weekly` borrowed across providers. Found beside it: all 684 Requesty models catalogued as free. **M8-6**, `model_plan.md` §1–2. |
| 2026-09-15 | **D153** — **the keychain was trusting Node, so the shell holds it now.** The entry for a real OpenRouter key trusted exactly one program, `alexia-core` — which is Node, so it trusted every script Node runs: `alexia-core -e` with two lines read the key back with no prompt, and every `"run": "node"` plugin is started with that binary. Custody moves to `src-tauri/src/vault.rs` (service `dev.alexia.app`, get/set/delete by name, no listing) behind a loopback port and a token written down core's stdin; `boot.mjs` awaits `fromShell` before serving, so a shell that hands over nothing is a core that does not start. The sidecar starts with `NODE_*`/`DYLD_*`/`LD_*`/`OPENSSL_*`/`SSL*` scrubbed and `--disable-sigusr1`, because either would put code inside the process holding the token. `custody()` moves old entries on first read — copy, then delete, by core, whose program they trust — so there is no prompt unless Node changed since the entry was written, and then one, as a Node update always cost. `package.mjs`'s smoke test and the release's Intel job start a core under `ALEXIA_TAURI` with no shell, so both now hand it a stand-in vault. Rust goes from 208 to 279 of 300. Verified on a Mac with the real key: moved after one prompt (Node 26.8 against the installed 24.20), old entry gone, new entry trusting only the shell's hash, OpenRouter still connected, and `alexia-core -e` stalled on a dialog instead of reading. Also found by running it: the core is frozen while that prompt is up, because `cross-keychain` reads synchronously — true of any prompt before, now once per install where Node changed. **Not closed:** an ad-hoc signed shell is pinned by hash, so each new build is allowed once; the bundle's scripts are user-writable. Both are Developer ID. Windows and Linux stores have no per-program access list, so there it is a code path, not a wall. |
| 2026-09-14 | **D148** — **computer control on a Mac, through the accessibility tree and Quartz events, with no native module.** `macos.js` answers every call `windows.js` does, the same bargain one platform over: `osascript`'s JavaScript reaches Objective-C and C directly, so Quartz events move the pointer and press keys, the accessibility API is the control tree, and `screencapture` takes the picture — one spawn an action, nothing compiled. **`desktop.js` picks the backend and imports nothing else**, so `replay.js`'s promise that a plan cannot reach a model is still proved by walking imports, one hop longer. **Key notation is the backend's to define**: SendKeys' `^c` means nothing on a Mac, so each backend exports the `key` tool's description and a Mac is taught `cmd+c`, with Windows notation refused by name. **Four things were found by running it.** Typing a string through the bridge sent nothing — every keystroke kept key code 0 and *žluťoučký* arrived as `aaaaaaaaa` — because a JavaScript string does not cross into a `UniChar *`; the characters now go as UTF-16 bytes through a bound pointer, measured by reading each event back before posting it. A letter is a key *on this keyboard*, so `cmd+z` finds the key that types `z` by asking every key code, and undoes on a Czech layout. A raw out-parameter cannot be handed back to the accessibility API while the same value wrapped can, and `activationPolicy` arrives as a string. And Calculator's display says `\u200e9`, so direction marks are stripped before a check compares. **Verified** on Apple Silicon with Accessibility allowed: Calculator's 163 controls listed, AllClear 7 + 2 = pressed without the pointer and `9` read back; Czech text typed into TextEdit, `cmd+a`, `cmd+z`, a click, and the pointer put back. **Not verified**: the screenshot, because the terminal it was tested from has no Screen Recording permission and granting one means restarting it — the refusal names the setting. macOS only reports windows on the Space in front, which is a limit of the platform and not of this file. |
| 2026-09-14 | **D147** — **Whisper and Piper for a Mac are built here, because neither project publishes one that runs.** whisper.cpp's releases carry an xcframework and no program; Piper's `piper_macos_aarch64.tar.gz` is an **Intel binary** — *bad CPU type* on Apple Silicon — whose libraries are not beside it and which has no rpath to find them with. `scripts/voice-macos.mjs` builds both from the pinned tags for the architecture it runs on: SDL2 static so `whisper-stream` carries its microphone code, `GGML_NATIVE=OFF` so a newer runner does not emit instructions an older Mac lacks, Metal shaders embedded, the phonemiser pinned instead of Piper's `master`, and every library Piper loads through `@rpath` copied beside it — read off `otool -L` rather than listed, because the pinned tag names espeak differently from `master` and splits `libucd` out, and a list written against one packages a Piper that cannot start against the other. It refuses to pack anything that loads a library from the build machine, and it makes Piper say a sentence first. `voice-macos.yml` runs it on one Apple Silicon and one Intel runner and drafts the release `fetching.js` names — `make_latest: false`, so the app's updater is never pointed at it. **Verified** on Apple Silicon: the script from nothing in 216 s; the plugin's own `install()` against those archives; Piper speaking and Whisper hearing it back word for word, on the GPU. **Not verified**: the Intel build, the workflow, and the microphone — `whisper-stream` needs Alexia to be allowed the microphone, which `Info.plist` now asks for. The downloads answer 404 until somebody publishes the draft. |
| 2026-09-14 | **D146** — **text in pictures on a Mac, through Vision.** The same trade `windows.js` made: the recogniser the OS already has, reached from a script, nothing downloaded. `macos.js` runs `VNRecognizeTextRequest` from `osascript`'s JavaScript and hands back lines with pixel boxes the other way up from how Vision measures them, so `lines.js` sorts both platforms identically. **Found by running it**: a page another plugin hands over is spilled to a file with no extension, and every AppKit call that takes a path picks its decoder by extension — so the scan was *not a picture*; it is decoded from the bytes now. And the manifest's reason for `proc.spawn` grew past the schema's 120 characters, which the loader refused with the plugin simply absent — so `documents` said *nothing installed here does that*, which was true for a reason nobody would guess. **Verified**: `ocr.test.ts`'s whole chain on a Mac — a scanned PDF refused by `documents`, the page handed over as bytes, read by Vision, two columns back in reading order — and 30 languages listed in the reader's own, Czech among them. |
| 2026-09-14 | **D145** — **the Mac app: a hotkey that is not the system's, a reopen that works, and a build that runs under the hardened runtime it would ship with.** `Ctrl+Option+Space` is macOS's own *next input source*, so the Mac registers **Option+Space**, and `desktop.ts` names it — with a test holding the page to what `main.rs` registers. `RunEvent::Reopen` brings the window back, because a second launch on a Mac never becomes a process and single-instance never hears of it. `--overlay` summons the overlay from anything that can run a command. The overlay is `CanJoinAllSpaces` and `FullScreenAuxiliary` — one AppKit call through `objc2-app-kit`, already under Tauri. **The Dock icon follows the main window**, as the taskbar entry does on Windows: in the Dock while the window is up, a menu-bar item while Alexia is only a daemon. It was taken out once and put back. Alexia kept being terminated after its window closed, and the toggling was blamed — but the cause was **a utility on the test Mac (Vorssaint) that quits any app whose last window closes**, which is why removing the toggle changed nothing. Found by the person at the machine, and fixed there with an exception for Alexia; any Mac with such a utility needs the same. Worth writing down for the cold-install test: *Alexia quits when I close the window* on somebody's Mac is that before it is a bug. **Signing**: `Entitlements.plist` gives Node the JIT and unsigned memory it cannot start without, library loading for plugins' own addons, and the microphone; `Info.plist` asks for the microphone in a sentence; `sidecar.mjs` signs the keychain library, because Tauri signs executables and nothing under `resources`; local builds sign ad hoc with the hardened runtime, releases with `APPLE_SIGNING_IDENTITY`. `release.yml` gains a Mac job per architecture that signs, notarises and staples when `sign` is on, and **`latest.json` moved into a `publish` job**, because it is one file listing every platform and two jobs writing it would publish whichever finished last. `ci.yml` runs on macOS. **Verified**: the signed `.app` and `.dmg` built; `codesign` strict-valid with the runtime flag and all four entitlements on the sidecar; hardened Node running JIT and loading the keychain; the window closing to the Dock, reopening, `--overlay` with the window up, and the overlay hiding on click-away — each across repeated runs, with no exit. Also verified, by the person at the machine and again by automation: with the window closed Alexia stays in the menu bar and Option+Space brings up the overlay; the page reaches the shell over IPC on macOS (the startup update check is the witness — the shell connects to GitHub only when the page asked it to). D66's blur guard, owed since M5-2, is built. **Verified not to work: the overlay over another app's full-screen Space.** TextEdit put into full screen, confirmed by its own `AXFullScreen`; Option+Space brought Alexia to the front in menu-bar mode, and the overlay did not appear on that Space. `CanJoinAllSpaces`, `FullScreenAuxiliary`, the floating level and the accessory policy are all in place and are not enough for an ordinary window that activates its app. The approach that is known to work is a **non-activating `NSPanel`** — what `tauri-plugin-spotlight` does, and what M2-S1 already named — and it means swapping the overlay's AppKit class in unsafe Rust, in a file whose charter is to stay boring. That is the owner's call, not a fix to slip in. **Not verified**: notarisation, which needs an Apple Developer account this project does not have; the release workflow, which nobody has run; and Escape over the overlay under automation, which gave no usable reading. Build note: this Mac's Command Line Tools carry a 27.0 SDK their own linker cannot read, so builds here set `SDKROOT` to 26.5. |
| 2026-09-14 | **D144** — **Alexia runs on a Mac, and a fresh install on any platform was never watching its plugins folder.** Answered by running it on one (Apple Silicon, macOS 26.6) rather than by reading. **Core needed nothing**: typecheck, the unit suite and the invariants pass as they stand, and the data folder lands in `~/Library/Application Support/Alexia` because `store.ts` already said so — invariants 6 and 7 held, which is what the Backlog's *a port, not a rewrite* was conditional on. **The packaging did not survive, in exactly D75's shape.** The keyring slug was `darwin-universal`, which `@napi-rs/keyring` 1.3.0 has never published — one package per architecture — so `package.mjs` threw before copying anything, and `sidecar.mjs` listed the same missing name. The runtime was `node.exe` and the launcher `.cmd` everywhere. The browser was opened with `cmd`, and a missing command is an `error` **event** rather than a throw, so the `try` around it caught nothing and the double-click build would have died a moment after printing its address. And the build's self-check pointed only `LOCALAPPDATA` at its throwaway folder, which a Mac ignores, so it would have checked against the real install. Now: slug by platform *and* arch, `node` and `Alexia.command`, `open`/`xdg-open` with the error heard, `HOME` and `XDG_DATA_HOME` beside `LOCALAPPDATA`, and `tauri.macos.conf.json` building an `.app` instead of NSIS. The self-check also sets `ALEXIA_TAURI`, so `pnpm package` no longer opens a browser on the build machine — Windows included. **No Rust changed.** **Running it found a bug that is not the Mac's**: `extensions` does not exist until something is installed — every install since D118 — and `Plugins.watch()` on a missing folder fails once, as invariant 2 requires, and never retries; so a plugin folder deleted by hand went unnoticed until a restart, and `/api/plugins` said *cannot watch for changes* on every first run. `serve()` owns the data folder and now makes the folder before watching it; the test was proved failing against the old file first. **Verified**: `Alexia.app` built and launched; one shell, one sidecar core, its page loaded in the window; 403 without the token and 200 with it; a data folder made from nothing with `problems: []`. **Left for D145–D148:** signing and notarisation, the Dock and reopen behaviour, the hotkey that collides with macOS's *next input source*, the overlay over full-screen Spaces, and the three plugins that were Windows code. **Corrected the same day:** this entry first said a launch from a background process builds the windows without showing them. It does not — the window was on another desktop, and the window list it was read from only reports the Space in front. The person at the machine said so, which is the only reason it was caught. Build-machine note: this Mac carries an SDK newer than its linker reads, so `SDKROOT` had to name 26.5. |
| 2026-09-02 | **D143** — **a plugin had two homes, and neither screen said which held what.** Reported from use. `plugins/voice` put the values you set on one screen and the things those values drive on another: the recording to clone and the words in it were typed under Settings → Plugins → Voice, and the button that reads them was a tab on the control surface — so two of that plugin's own hints had to end *on the plugins screen* and *on the Voice panel*, which is a manifest apologising for the shell. **The split was real and it was in the wrong place.** *Values you set* and *a record you read* are genuinely two things, and D86 made them two lists for that reason; what does not follow is two screens, because somebody opening either one is asking about **the plugin**, not about which half of it. So `panel` stays exactly where it is in the manifest — no revision, nothing to migrate, the three other manifests that declare one untouched — and `pane()` renders its widgets alongside the settings, which the plugin's page draws under a rule with the author's `label` above only where that is not the plugin's own name. **The founding property is what made this cheap rather than risky**: the plugins page was already assembled from manifests exactly the way the tab list was, so no file in core or the shell gained a plugin's name, and M6-G's gate reads the same one screen over — delete the folder and the whole plugin goes. Three things fell out. The palette had **two rows for one plugin** and now has one, because `library` already listed every installed plugin and pointed at the page that now holds all of it. `Tab` lost `from`, `plugin` and `running`, `tabs()` stopped taking manifests and stopped being async, and `/api/panels` is a read of the store — the control surface is core's own and says so in its types. And **voice's manifest moved three widgets rather than being left correct-but-strange**: `clip`, `clip_text` and `add_voice` now sit next to the buttons that read them, so a hint can say *above* and be right, which is the whole of what the change was for. The one thing deliberately not done is collapsing `settings` and `panel.widgets` into one list — that is a protocol revision and a migration for every plugin, to buy an ordering an author can already have by choosing which list to declare in. |
| 2026-09-02 | **D142** — **both widgets that fill themselves from a tool were unreachable, and one had been since the day it shipped.** `/api/rows` finds a widget by its key and reads the tool off it, and the finder was a list of two type names — `table` and `graph`. `image` was added at D132 and `cards` at D141, and **neither was put on that list**, so core answered *there is no list called that* for both. The picture gallery had been saying it since D132; nobody had reason to open that panel twice, which is exactly how a screen stays broken. **The shell had a second bug on the same path**: `image` and `cards` sent the *rows tool's name* where the route wanted the *widget's key*, so even widened they would have missed — `made` draws from `pictures`, and core was asked for a widget called `pictures`. Both are fixed at the root rather than at the two call sites: the finder is now *anything carrying a `rows` tool*, written as the rule instead of as a list somebody must remember to extend, and every widget asks by key. **The check is an invariant, not a unit test**, because it is one line of arithmetic against every manifest in the repo and it fails on the day a fifteenth widget grows a `rows` and forgets — rather than on the day somebody opens the panel. Proved by putting the old rule back and watching it name both: *media: image “made”* and *media: cards “library”*. |
| 2026-09-02 | **D141** — **`cards`, the fourteenth widget, and `alexia_protocol` 6 — C2 built after all.** D140 answered *browse the library* with a `table` and argued C2 was a look rather than a capability. **Using it settled that**: forty rows of a spreadsheet are forty things nobody can judge, and the plugins page answers the same question with a name, a sentence and a state that people already read one screen over. That is a difference in what a reader can *do*, which is the bar `ui-schema.md` sets, and the earlier judgement was wrong. **Almost none of it is new drawing**, exactly as §8.1 predicted: `bento` is the grid, `bento-card` the card, `bento-label` the row that explains a section and `dim` what D120 settled about showing what you do not have. What is new is that the cards come from a plugin's tool rather than from the folder — the generalisation `settings.ts:16` was one word away from, so core learns that a plugin contains things and never learns the word *workflow*. A row is `id`, `name`, `summary`, `meta`, `state` and `group`, fixed by the contract the way `graph`'s is. **`when` on a row action is the one addition that is not a rename**: *Install* belongs on what is not here and *Remove* on what is, and without it both sit on every card, which is how somebody presses Remove on something they never installed. **The small size had to be laid out rather than left to wrap** — a pill, what it needs and a button do not share eleven rems, and letting all three wrap made every card three rows tall and the grid stopped being scannable, which is the one thing it is for. So the state and the button share a line and the needs go under it, clipped. Measured on this machine: 49 cards — four of them yours, 45 the card can run — over the honest sentence, 468 shipped, 255 paid, 45 that fit. |
| 2026-09-02 | **D140** — **the library is a page, and it is the third tier only.** §8.1 wanted the plugins grid applied one level down and §8.4 ranks three tiers — *Verified*, *Curated*, *Templates*. **Two of them do not exist**: nothing records what the owner has run and tagged, and no Alexia set is published. §8.4 says a badge must record what it was verified against *or it rots into authority it has stopped deserving*, so inventing the first two tiers would have been exactly the rot it warns about. The page is the third tier, ComfyUI’s own, and the hint says so rather than implying a ranking that is not there. **It needed no core work.** The bento grid C2 proposed is a look; `table` already carries rows from a tool, an action per row, a detail view, a filter and grouping — which is the whole of *browse the library and install one*. **Filtered on the machine before it is ranked on the words** (D133): 45 rows out of 468, nothing paid, grouped by the catalogue’s own categories, installed ones first because that is what somebody opening it is looking for. Measured live: 45 across seven groups, every row carrying the id a row action needs and every declared column a value. |
| 2026-09-02 | **D139** — **a workflow may come from a link, because the rule forbidding it was written about something else.** §8.6 states it flatly: *never a free-text search, never a pack the model chose, **never a URL from a conversation***. That rule exists because **installing a node pack is running a stranger's Python**, and it was the right rule for that. It is not the right rule for a workflow: D137 established the risk is not reachable from here, since the converter refuses any node class this machine does not already have — so what a link yields is text that either maps onto nodes already installed or is turned away by name. **A link is exactly as dangerous as a file on disk, which `add_workflow` has always accepted**, and keeping the ban would have meant *download it yourself, then tell me where you put it* for no safety anybody could point at. **What is enforced instead is the shape of the thing.** `https` only, because a workflow fetched over plain text is one anybody on the path can rewrite and it ends up queued on the person's own machine; a size ceiling, because an address answering with a gigabyte is not a workflow and should cost one refusal rather than a disk; and **the commonest mistake is named rather than reported** — a link to the *page* showing a workflow rather than to the file comes back as *that host answered with a page*, not as a parse error that sends somebody hunting for a broken workflow. **The two sources were checked rather than assumed.** `comfy.org/workflows` is not a second catalogue: it is the web front of the `comfyui-workflow-templates` package ComfyUI ships, which is what `find_workflow` already reads. The 626 there against 468 here is **a stale pin, not a missing feature** — this install holds 0.11.9 from 13 July and PyPI is at 0.11.52, 198 versions on. RunComfy stays not a source, and the second look agreed with the first: sign-in throughout, no public API, redistribution terms unstated. **Measured end to end**: a raw workflow off GitHub fetches, converts, lands as both halves unstale, and comes back steerable with `prompt`, `avoid` and `model` through D138 — and `comfy.org/workflows` itself is refused with the sentence about pages. |
| 2026-09-02 | **D138** — **the wiring answers what the titles do not, and a model swap is offered with its cost written on it.** D128 offers a field because its author *named the box*, and that is right and stays: a graph with its own vocabulary keeps it. But **ComfyUI’s own catalogue templates rename nothing**, so every one of the 468 arrived through D137’s `install_workflow` with zero fields and ran with whatever prompt its author baked in — which makes *install this and run it against what I asked for*, the one journey install exists to serve, impossible for the entire catalogue. So when a workflow names nothing, the graph’s own wiring is read instead. **It is a derivation, not a heuristic about names.** A sampler is any node taking both a `positive` and a `negative` conditioning link — read off shape, because a hardcoded `KSampler` misses every custom sampler and this install has several; *which is which* is stated by the node consuming them, so nothing is inferred from the words in the box; and the text box is found by its declared type, `STRING` with `multiline`, out of the same `/object_info` everything else is read from. The model is the same walk: follow `model` while it is a link, because that is how LoRA and patch loaders chain, and stop at the node choosing a file. **Every ambiguity returns nothing** — two samplers, two text boxes, no text box — and the positive box is required, because handing back `avoid` alone claims a workflow can be steered when the thing it makes cannot. **Then it was run, and the run is the finding.** SDXL Turbo installs, takes the prompt it is given, renders — and against a substituted checkpoint returns **grey mush**, because a Turbo workflow is one step at guidance 1, tuned to its own distilled model. The template names `sd_xl_turbo_1.0_fp16.safetensors` and this card holds six other checkpoints and none of them is it, so without the swap the whole catalogue is unusable here and with it the picture can be ruined. Both are true, so the swap is offered and **the cost is stated where the choice is made**: a tool that quietly returns a ruined picture is worse than one that says this may not work. |
| 2026-09-02 | **D137** — **installing a catalogue workflow needs no consent ladder, because the converter already refuses everything dangerous.** §8.6 calls this *the sharpest edge in this document*: a workflow is JSON and harmless, but **the node packs it needs are arbitrary Python that runs on the person's machine with the person's files**, and the owner chose *one consent, then trust the catalogue* — the lowest-friction answer and the highest-risk decision in the plan, propped up by a rule that nothing outside a vetted entry may ever be installed. **Building it revealed the risk is not reachable from here.** D134's converter refuses any node class this ComfyUI does not already have, by name; so a workflow that installs cleanly is, definitionally, one whose nodes are **already on this machine**. `install_workflow` downloads no pack, executes nothing, and writes one JSON file. The consent that decision was buying would have been asked for a power the tool does not have, which is worse than not asking — it teaches somebody that the question is noise. **What survives from §8.6 is the part that was never about consent**: the plugin's page lists what was installed and can remove it, because a silent tenth install is only acceptable if the ninth is still visible. Measured live: 468 entries, 45 runnable on this card — D133's figures unchanged — and of the convertible ones SDXL Turbo installs by its title with both halves on disk and unstale. **It arrives with no knobs, and that is D128 working rather than failing**: only fields whose nodes an author renamed are offered, and ComfyUI's own templates leave every node at its default name — so a template runs with what its author baked in, and the tool says so rather than implying settings that are not there. |
| 2026-09-02 | **D136** — **the pipeline is a strip rather than a graph — and both things §7.3 wrote off come back.** `comfyui_engine_plan.md` §7.3 named two losses and said neither should be papered over: **`mark` is one boolean, so per-node percentages cannot be drawn**, and **the force layout is tuned for a relationship graph while a workflow is a pipeline** — *a real mismatch, not a tuning problem*. Both were consequences of one assumption, that the diagram would be the `graph` widget. **Neither survives dropping it.** A pipeline has one dimension, so drawn as segments in execution order it needs no layout at all, and each segment having its own width *is* the per-stage percentage the boolean could not carry. So `alexia/stages` rides the progress notification beside D135's `alexia/preview`: no widget change, no `force.ts` change, no `alexia_protocol` bump, and the memory plugin's cloud is untouched — a stages mode on a canvas widget with no second user would have been the larger diff and the worse picture. **The order is the plugin's and core never sorts it**, which was measured rather than assumed: `progress_state` names only the nodes that have *started* — the first frame of a real render named exactly one — so a strip built from the socket would grow a segment at a time and draw the reporting rather than the pipeline. The plugin owns the graph and lays it out with Kahn's before anything runs; two nodes at the same depth may fill in an order ComfyUI does not run them in, which is a fair reading of a pipeline that has no order between them. The per-node data is real and was read off `comfy_execution/progress.py`: `pending`, `running`, `finished`, `error`, each with its own `value` and `max`. **No names are drawn on it.** Five fit across a 25.5rem rail and twenty-five do not, and a strip that fails exactly where a pipeline got interesting is the wrong trade — the line above already says *KSampler — step 12 of 20*, so the strip adds the shape and the label goes on `title`. Core refuses a malformed strip **whole** rather than drawing part of one, caps it at 64 and cuts a label at 80, because a `state` becomes a class name and a label becomes text: a pipeline with a step missing is a wrong picture, where no pipeline is the bar every plugin already gets. **Two things were found by running it rather than reasoning about it.** A finished job never closed its own strip — `/history` ends a job before the socket's last `progress_state`, and a real render stopped on `█████▶·` with the picture already saved, an unfinished pipeline under a row reading *done*; a successful wait now sends the closing frame itself. And **D135's picture was laid out wrong the moment it shipped**: a rail row is a flex line of text, so a 14rem image beside the step name left the name nothing and pushed the status off the edge. Rendering it in a browser showed in one screenshot what the working data path had hidden. The work — picture and pipeline — now goes on its own line beneath the row. **And `progress()`'s fifth argument became `{ preview, stages }`** rather than growing a sixth positional: four optional slots in a row was already the limit, and nothing had shipped against it yet. |
| 2026-09-01 | **D135** — **the picture, while it is still being made — and the one-word setting that made the socket look broken.** D129 opened the websocket and read the three JSON messages off ComfyUI's source; the **binary** frames on that same socket are the picture as it stands, in two layouts also read off `server.py` rather than documentation — `send_image` (event 1) is `[event][1 = JPEG or 2 = PNG][the image]`, and event 4, sent only to a client that asked for it by feature flag, is `[event][length][JSON][the image]` with the mimetype in the JSON. **The first attempt received zero of them, and nothing was wrong with the code**: `--preview-method` defaults to `none`, so ComfyUI computes no preview and the socket that D129 proved was listening had nothing to hear — the same silence-that-reads-as-a-slow-card shape as D129's `client_id` bug, one layer further out. ComfyUI-Manager no longer offers a runtime override for it (its own startup line names PR #11261 and points at the CLI option), so **the flag at launch is the only lever** and `launch.js` now passes it. **`latent2rgb` rather than `auto`**: `auto` prefers TAESD, a model that has to be downloaded and then decoded once per preview *on the card already busy generating*, while `latent2rgb` is a matrix multiply on a latent that is already in memory — a rougher picture for almost nothing, which is the right trade for something glanced at while waiting. **A `data:` URL and never a path**, because a frame exists for a second and is replaced, and writing each to disk to serve it back would be a file per step of every render; capped at 400 KB where it is produced, because a progress channel is not a transport. It rides under `_meta['alexia/preview']` — the same door `alexia/tools` and `alexia/files` go through — so **there is no protocol bump**: an Alexia that has never heard of a preview draws the bar and ignores the rest. Core drops anything that is not a `data:image/` URL, checked at the boundary *and* again in the shell, because this is the one field a plugin fills that becomes an `img` source and it does not get to name a path or a host. The shell keeps **one** element and replaces it, rather than appending: a render sends one of these a second and a filmstrip is the wrong picture of a thing that has a single current state. **Measured on a 20-step render: 16 frames, JPEG, 35 KB growing to 98 KB** as it sharpens. |
| 2026-09-01 | **D134** — **a converter that refuses instead of guessing — and the measurement that says how far it gets.** D123 refused to re-implement `graphToPrompt` because *a converter that handles the plain cases and fails on these is worse than none, because it fails silently and specifically on the interesting graphs*. Every word of that stands. **What changed is that silence is not the only option**: widget order is not a heuristic but `/object_info`'s own `input_order`, with a slot inserted after any widget marked `control_after_generate` — so the expected number of values is a *number*, and comparing it with the array's length is arithmetic. Every uncertainty is a refusal that names the node: a bypassed node, a Reroute, a subgraph, an uninstalled class, a count that does not add up. **Then it was measured against all 45 templates this card can run, and it converts 6.** Twenty-five stop on a subgraph — which §4.1 predicted as *not in these three, but one save away* — thirteen on a widget count, one on a Primitive. **That figure is the finding rather than a disappointment**: a hand-written conversion recovers about one workflow in seven and the rest still need the frontend that has always owned this, so D123's refusal is now vindicated by numbers instead of by argument. Six with proof beats none, and `add_workflow` tries before it refuses rather than sending everybody back to a menu. **It also found a bug in shipped code**: this install spells a combo two ways at once — older nodes put the option list where the type goes, V3 nodes say `COMBO` and move the list into the options — and `workflows.js` read only the first, so a V3 node's field came back as free text with no list behind it, which looks like it works and quietly stops validating what somebody typed. |
| 2026-09-01 | **D133** — **ComfyUI ships the workflow catalogue, and the honest number is not the big one.** `GET /templates/index.json` on the running server answers with 468 grouped entries carrying a title, description, tags, the models each needs, **how much video memory it wants**, and whether it is open source — so nothing has to be scraped, mirrored or kept current, and it moves when ComfyUI moves. RunComfy was checked and is not a source: sign-in for everything, no public API, redistribution terms unstated. **Two numbers in the plan were wrong and both were too flattering**, read off the local pip manifest instead of the live index: it is 468 rather than 484, and **255 of them call paid services rather than 103** — more than half. Worse, of the 213 open-source ones only **45 fit an 8.5 GB card**, and four of those make images. So the catalogue filters on the machine before it ranks on the words, and the sentence it leads with is the count about *your* card rather than the total: *468 workflows* to somebody who can run 45 is accurate and misleading at once. **The search needed a floor, found by using it**: without one, *read this out loud* came back with depth estimation and an anime model — entries sharing a stray word with a paragraph. A tag hit or better, **or** three words matching a description, because *make a picture from words* and *Text to Image* share no word at all and a three-word agreement is not coincidence. Finding works; **installing does not yet** — the templates are editor format, so it names the entry and the one menu click rather than pretending. |
| 2026-09-01 | **D132** — **`image`, the thirteenth widget, and `alexia_protocol` 5.** The twelve could describe a picture and could not show one. **This is not the `file` widget refused twice at D89** — that was a plugin asking somebody to *choose* a file, which a browser genuinely cannot express; this is a plugin that has *made* something and has nowhere to put it. Granted the way `graph` was, on what the alternatives cost: the shell drawing pictures for one named plugin is this project's founding complaint arriving by the back door, and a sandboxed iframe hands over the palette and the focus ring with the pixels. **And it arrives with three users where D115 was uneasy about having one** — a gallery, a thumbnail on a library item, and a preview of something still being made. A plugin says what the pictures are and what each is called; size, fit, the gap while one loads, the alt text and the dark-mode behaviour are all core's. **The route is the safety and it needed its own boundary**: `/api/file` serves what a *tool result* offered, from a list core wrote down, but a widget's rows are paths the **plugin** chooses — so `/api/plugin-file` will read inside the asking plugin's own folder and nowhere else, with `realpath` on both sides before the comparison, because `..` and a symlink are the two ways a path that reads as inside points outside and a string prefix test catches neither. First user is the picture plugin's **Pictures** panel, which puts the folder's size on screen beside it: keeping every picture for ever is a decision, and a folder that only grows out of sight is the dishonest version of it. |
| 2026-09-01 | **D131** — **a conversation ending is something a plugin can be told.** `plugins/media` starts ComfyUI detached and lets it outlive the plugin on purpose — loading PyTorch and a checkpoint costs the better part of a minute, and this plugin is stopped after five idle minutes, so tying them together would mean paying that minute after every pause. The cost of that decision was that **nothing ever said *stop***: a graphics card stayed occupied all night because somebody asked for one picture at lunchtime. Starting a new conversation is the clearest signal a person gives that they have finished, and it is the only one that arrives without anybody remembering a command. So `alexia/conversation/ended` is broadcast to every running plugin. **It carries nothing about the conversation** — not its id, not what was said, not how it ended — because a plugin has no business knowing, and one that could tell two conversations apart could keep a record of them. Nothing is spawned to hear it, nothing waits for an answer, and ignoring it is correct for almost every plugin; the picture plugin stops the ComfyUI **it** started, under the same two conditions `stop_comfyui` uses, because finishing a chat is not permission to close somebody else's program. Fired from `surface.ts`'s own `newChat`, below its early return, so pressing New twice does not announce an ending that did not happen. |
| 2026-09-01 | **D130** — **the picture plugin is called *Local media generation* now.** §13.5 of `comfyui_plan.md` deferred this until a workflow that is not a picture actually ran; `comfyui_engine_plan.md` makes that certain rather than likely — the catalogue has voice and video in it by design, and one of the three workflows saved on this machine renders audio. The engine is named in the summary rather than the title, so the card reads for the person the setup promise is written for and still says *ComfyUI* to anyone looking for it. **The id and the storage namespace stay `media`**, because `store.ts:141` migrates neither and a rename that moved them would orphan every picture already made. |
| 2026-09-01 | **D129** — **the websocket, adopted — and `comfy.js:11` argued the other way on purpose.** That comment chose polling `/history` over the socket: *a dependency and a reconnect story less for a job that takes twenty seconds*, at the cost of progress in whole images rather than steps — and it named its own successor, *the websocket is the upgrade and it is the only thing that would change here*. Per-step progress is websocket-only (`main.py:437`, `execution.py:494`, `comfy_execution/progress.py:184`), and a person watching a thirty-second render wants to know it is at step 12 of 20 rather than that something is happening. **`/history` still decides the job is finished**; the socket only makes the waiting legible, is opened per job, is allowed to fail, and degrades to precisely the old behaviour — which is the reconnect story the original comment was avoiding, and there isn't one. **The bug this was built around is silent and is now structurally impossible.** ComfyUI sends every progress message to the queueing client and nobody else, keyed by the socket's `clientId` query parameter (`server.py:273`) — so a socket opened with one id and a prompt queued with another connects, handshakes, and receives nothing, which is indistinguishable from a slow graphics card. There is one `CLIENT_ID` per process used by both, and `queue()` no longer accepts one, so a caller cannot get it wrong because a caller is not asked. Its test asserts a message **arrived**, not that a socket opened, and was checked by deliberately mismatching the ids and watching it fail. The trace panel shows the plugin's sentence *and* the percentage now rather than replacing the first with the second — `43%` said less than *KSampler — step 12 of 20*, and that is generic rather than about this plugin. |
...
| 2026-08-31 | **D119** — **Alexia updates itself, in one press.** `tauri-plugin-updater` was already a dependency and had never worked: `pubkey` was the empty string, the endpoint named a domain nobody owns, and `active`/`dialog` in the config are Tauri **v1** keys that v2 ignores without a word — so no check ever ran and nothing would have installed if one had. A keypair, a `latest.json` on the app's release, and one `invoke` fixed it. The order in `release.yml` is the argued part: **Authenticode rewrites the installer**, so a minisign signature taken at build time describes bytes that no longer exist, and every update would be refused with an error nobody can act on. Build → SignPath → `tauri signer sign` → `latest.json`. Plugin releases are cut `--latest=false` for the same reason from the other side: the updater reads `releases/latest/download/latest.json`, and a plugin release claiming to be latest points every install at a release with no installer on it. |
| 2026-08-31 | **D118** — **nothing ships inside the installer, and the shelf is GitHub Releases.** M5-4 rebuilt, and `SHIPPED` went from eight names to one to none. Shipping our own plugins made them privileged by the mechanism rather than by merit, which is this project's founding complaint by the back door — and `hello`, kept a milestone longer as the offline proof that installing works, was an argument about the developers rather than the user. What replaced it is a first-run step that reads the shelf and asks *what should it be able to do?*, with each plugin's own `requires` sentences under the tick that agrees to them. A release carries the `.tgz` as an asset and the entry as a fenced block in its notes, so the whole shelf is one API call and publishing is `gh release create` — no index, no deploy, no Worker that was never deployed. `min_app` is the new manifest field: `alexia_protocol` is the shape of the contract and cannot say *needs a capability that arrived in 0.2.0*, which is a failure that could not exist while everything shipped together and is the first one that appears once it does not. The static layout is kept behind `--pages` for the one thing GitHub cannot do — a revocation that reaches somebody who already installed it. |
